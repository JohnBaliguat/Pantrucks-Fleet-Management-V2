            ...
            <div class="col-12">
              <div class="card">
                <div class="card-body">
                  ...
                  <p class="text-muted mb-3 fs-3">Sum of <code>trips.piece_rate</code> for each driver. Rates come from Trip Rates and are stamped at dispatch time.</p>
  ...
  <script>
    (function () {
      ...
      function load() {
        ...
        const to   = $('#dateTo').val();
        $.getJSON('php/fetch/get_payroll_data.php', { date_from: from, date_to: to })
          .done(function (resp) {
            if (!resp.success) {
              Swal.fire('Error', resp.message || 'Failed to load', 'error');
              return;
            }
      ...
      $('#payrollFilter').on('submit', function (e) { e.preventDefault(); load(); });
      load();

      window.showDetail = function (driverId, name) {
        const from = $('#dateFrom').val();
        const to   = $('#dateTo').val();
        $('#detailTitle').text(`Trips: ${name} (${from} → ${to})`);
        $.getJSON('php/fetch/get_payroll_data.php', { date_from: from, date_to: to, driver_id: driverId })
          .done(function (resp) {
            ...
            (resp.details || []).forEach(t => {
              $body.append(`
                <tr>
                  ...
                  <td>${escapeHtml(t.container_activity || '')}</td>
