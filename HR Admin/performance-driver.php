...
<body>
    ...
    <div class="body-wrapper">
      ...
      <div class="body-wrapper-inner">
        <div class="container-fluid">
          <div class="card mb-3">
            <div class="card-body">
              <div class="d-flex flex-wrap justify-content-between align-items-start gap-3">
                <div>
                  <h4 class="card-title mb-1">Driver Performance Breakdown</h4>
                  <p class="card-subtitle mb-0">Detailed scorecard, charts, and recent trip activity for one driver.</p>
                </div>
                <div class="d-flex gap-2">
                  <button type="button" class="btn btn-danger" id="exportDriverPerformancePdf">Export PDF</button>
                  <a href="index.php?route=hra-performance" class="btn btn-outline-primary">Back to Performance</a>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="row g-3">
            <div class="col-lg-8">
              <div class="card"><div class="card-body">
                <h5 class="card-title mb-1">Trip Trend</h5>
                <p class="card-subtitle mb-3">Total vs completed trips across the selected period.</p>
                <div id="trendChart" style="min-height:320px;"></div>
              </div></div>
            </div>
            <div class="col-lg-4">
              <div class="card"><div class="card-body">
                <h5 class="card-title mb-1">Recommendation Basis</h5>
                <p class="card-subtitle mb-3">Attendance, completion, and active violations.</p>
                <div id="statusChart" style="min-height:320px;"></div>
              </div></div>
            </div>
            <div class="col-lg-6">
              <div class="card"><div class="card-body">
                <h5 class="card-title mb-1">Trips by Segment</h5>
                <p class="card-subtitle mb-3">Which hauling segments this driver worked on most.</p>
                <div id="segmentChart" style="min-height:320px;"></div>
              </div></div>
            </div>
            <div class="col-lg-6">
              <div class="card"><div class="card-body">
                <h5 class="card-title mb-1">Attendance Breakdown</h5>
                <p class="card-subtitle mb-3">Present, absent, and leave distribution.</p>
                <div id="attendanceChart" style="min-height:320px;"></div>
              </div></div>
            </div>
          </div>
          ...
          <div class="card mt-3">
            <div class="card-body">
              <h5 class="card-title mb-1">Recent Trips</h5>
              <p class="card-subtitle mb-3">Full trip history for the selected driver with server-side paging and search.</p>
              <div class="table-responsive">
                <table class="table align-middle mb-0" id="recentTripsTable">
                  <thead>
                    <tr>
                      <th>Trip ID</th>
                      <th>Truck</th>
  ...
  <script>
    ...
    function recommendationBadgeText(value) {
      return value || 'No Activity';
    }
    ...
    function initCharts() {
      trendChart = new ApexCharts(document.querySelector("#trendChart"), {
        ...
        noData: { text: 'No trip trend available' }
      });
      trendChart.render();

      segmentChart = new ApexCharts(document.querySelector("#segmentChart"), {
        chart: { type: 'bar', height: 320, toolbar: { show: false } },
        series: [{ name: 'Trips', data: [] }],
        xaxis: { categories: [] },
        colors: ['#0ea5e9'],
        noData: { text: 'No segment data available' }
      });
      segmentChart.render();

      attendanceChart = new ApexCharts(document.querySelector("#attendanceChart"), {
      ...
      statusChart = new ApexCharts(document.querySelector("#statusChart"), {
        ...
        series: [0, 0, 0],
        labels: ['Efficiency', 'Presence', 'Active Violations'],
          ...
          radialBar: {
            dataLabels: {
              total: {
                ...
                formatter: function() {
                  return document.getElementById('bdRecommendation').textContent || '—';
                }
              }
            }
          }
    ...
    function initRecentTripsTable() {
      recentTripsTable = $('#recentTripsTable').DataTable({
        processing: true,
        serverSide: true,
        searching: true,
        responsive: true,
        ...
        order: [[5, 'desc']],
        ajax: {
          url: 'table-fetch/performance-driver-trips.php',
          type: 'POST',
          data: function(d) {
    ...
    function loadBreakdown() {
      $.getJSON('php/fetch/performance_driver_breakdown.php', {
        driver_id: driverId,
        fromDate: fromDate,
        toDate: toDate
      }).done(function(res) {
        if (res.status !== 'success') {
          $('#recentTripsBody').html('<tr><td colspan="6" class="text-center text-danger">' + (res.message || 'Failed to load breakdown.') + '</td></tr>');
          return;
        }
        ...
        trendChart.updateSeries([
          { name: 'Total Trips', data: (res.trend || []).map(function(r) { return r.total; }) },
          { name: 'Done Trips', data: (res.trend || []).map(function(r) { return r.done; }) }
        ]);

        segmentChart.updateOptions({
          xaxis: { categories: (res.segments || []).map(function(r) { return r.segment; }) }
        });
        segmentChart.updateSeries([{ name: 'Trips', data: (res.segments || []).map(function(r) { return r.count; }) }]);

        attendanceChart.updateSeries([
          (res.attendance && res.attendance.present) || 0,
          (res.attendance && res.attendance.absent) || 0,
          (res.attendance && res.attendance.leave) || 0
        ]);
            ...
            radialBar: {
              dataLabels: {
                total: {
                  ...
                  label: 'Recommendation',
                  formatter: function() {
                    return recommendationBadgeText((res.driver || {}).recommendation);
                  }
                }
              }
            }
        ...
        statusChart.updateSeries([
          Math.min(parseFloat((res.driver || {}).efficiency_percent || 0), 100),
          Math.min(parseFloat((res.driver || {}).present_percent || 0), 100),
          Math.min((((res.driver || {}).active_violations || 0) * 25), 100)
        ]);
      }).fail(function() {
        if (!recentTripsTable) {
          $('#recentTripsTable tbody').html('<tr><td colspan="6" class="text-center text-danger">Unable to load driver breakdown.</td></tr>');
        }
      });
    }
    ...
    $(function() {
      initCharts();
      initRecentTripsTable();
      loadBreakdown();

      $('#exportDriverPerformancePdf').on('click', function() {
        var form = $('<form>', {
          method: 'POST',
          action: 'php/reports/performance-driver-report.php',
          target: '_blank'
        });
