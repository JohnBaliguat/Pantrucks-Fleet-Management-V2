              ...
              <div class="col-lg-4">
                <div class="modern-card">
                  ...
                  <div class="card-body scroll-area mb-5">
                    ...
                    $q = $conn->query("SELECT * FROM booking WHERE status='Active' AND quantity <> quantity_use AND costumer!='CTH' ORDER BY booking_date DESC");
                    while ($b = ($q)->fetch()) {
                    ?>
                      <div class="booking-item"
                        ...
                        <div class="booking-left">
                          <div class="row">
                            ...
                            <div class="col-md-12"><p><?= $b['container']; ?> (<?= $b['container_status']; ?>)</p></div>
                          </div>
        ...
        Swal.fire({
            ...
            html: `
              <div style="text-align:left">
                <p class="mb-2">Complete the unit checklist before marking this truck available.</p>
                <label style="display:block;margin-bottom:8px;"><input type="checkbox" id="chk-engine"> Engine / running condition checked</label>
                <label style="display:block;margin-bottom:8px;"><input type="checkbox" id="chk-tires"> Tires checked</label>
                <label style="display:block;margin-bottom:8px;"><input type="checkbox" id="chk-lights"> Lights checked</label>
                <label style="display:block;margin-bottom:8px;"><input type="checkbox" id="chk-body"> Body / trailer connection checked</label>
                <label style="display:block;margin-bottom:8px;"><input type="checkbox" id="chk-clean"> Unit cleared and ready for next dispatch</label>
                <textarea id="chk-remarks" class="swal2-textarea" placeholder="Remarks (optional)" style="display:flex;"></textarea>
              </div>
            `,
            icon: "question",
            showCancelButton: true,
            ...
            preConfirm: () => {
                const payload = {
                    engine_ok: document.getElementById('chk-engine').checked ? 1 : 0,
                    tires_ok: document.getElementById('chk-tires').checked ? 1 : 0,
                    lights_ok: document.getElementById('chk-lights').checked ? 1 : 0,
                    body_ok: document.getElementById('chk-body').checked ? 1 : 0,
                    cleared_ok: document.getElementById('chk-clean').checked ? 1 : 0,
                    remarks: (document.getElementById('chk-remarks').value || '').trim()
                };
                if (!payload.engine_ok || !payload.tires_ok || !payload.lights_ok || !payload.body_ok || !payload.cleared_ok) {
                    Swal.showValidationMessage("All checklist items must be checked.");
                    return false;
                }
                return payload;
            }
            ...
            const form = new URLSearchParams({
                ...
                engine_ok: String(result.value.engine_ok),
                tires_ok: String(result.value.tires_ok),
                lights_ok: String(result.value.lights_ok),
                body_ok: String(result.value.body_ok),
                cleared_ok: String(result.value.cleared_ok),
