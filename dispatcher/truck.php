  ...
  <body>
                ...
                <div class="card">
                  <div class="card-body">
                    <div class="d-md-flex align-items-center">
                      <div>
                        <h4 class="card-title">Truck Management List</h4>
                      </div>
                      <div class="ms-auto mt-3 mt-md-0">
                        <button type="button" class="btn btn-primary btn-sm" id="AddModal"><i class="ti ti-plus"></i> Add Truck</button>
                        <a href="dispatch-unitProfile" class="btn btn-primary btn-sm" hidden><i class="ti ti-plus"></i> Add Truck</a>
                      </div>
                    </div>
                      ...
                      <table class="table mb-0 text-nowrap varient-table align-middle fs-3" id="table-data">
                        <thead>
                          <tr>
                            ...
                            <th scope="col" class="px-0 text-muted">Assign Genset</th>
                            <th scope="col" class="px-0 text-muted">Assign Trailer</th>
                        ...
                        <tbody>
                          ...
                          $query = "SELECT * FROM units WHERE unit_name NOT LIKE 'GS%' ORDER BY unit_id DESC";
                          $result = $conn->query($query);
                          $count = 0;
                          while ($row = ($result)->fetch()) {
                            $count++;
                          ?>
                            <tr>
                              ...
                              <td class="px-0"><?php echo $row['unit_assigngenset']; ?></td>
                              <td class="px-0"><?php echo $row['unit_assigntrailer']; ?></td>
                              <td class="px-0">
                                <?php
                                ...
                                if ($status === "" || $status === "Good") {
                                  ...
                                  echo '<span class="badge bg-warning text-dark">Rescue Unit</span>';
                                } elseif ($status === "Shop Unit") {
                                  echo '<span class="badge bg-danger">Shop Unit</span>';
                                } else {
                              ...
                              <td class="px-0 text-dark fw-medium text-end">
                                <div class="d-grid gap-2 d-md-block">
                                  <button class="btn btn-success btn-sm" type="button" onclick="openEditPage('<?php echo $row['unit_id']; ?>');">Edit</button>
                                  <button class="btn btn-primary btn-sm editBtn" type="button" onclick="openEditModal('<?php echo $row['unit_id']; ?>', 
                                    '<?php echo $row['unit_name']; ?>', 
                                    '<?php echo $row['unit_std']; ?>',
                                    '<?php echo $row['unit_plate']; ?>',
                                    '<?php echo $row['unit_address']; ?>',
                                    '<?php echo $row['unit_status']; ?>');" hidden><i class="ti ti-edit"></i></button>
                ...
                <div class="row">
                  <div class="input-style-1">
                    ...
                    <div class="column">
                      ...
                      <input type="text" class="form-control" name="unit_plate" id="unit_plate" placeholder="Enter Unit Plate Number" required>
                    </div>
                    <div class="column">
                      <label for="unit_location">Location</label>
                      <input type="text" class="form-control" name="unit_location" id="unit_location" placeholder="Enter Location" required>
                    </div>
                    <div class="column">
                      <label for="unit_std">Unit Standard Ratio</label>
                      <input type="number" class="form-control" name="unit_std" id="unit_std" placeholder="Enter Unit Standard Ratio" required>
                    </div>
                  </div>
                </div>
              </form>
    ...
    <div class="modal fade" id="editModal">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h3 id="title">Update Unit Status</h3>
          </div>
          <div class="modal-body">
            <form id="editForm" action="php/updatetruck.php" method="POST" enctype="multipart/form-data">
              ...
              <input type="hidden" name="unit_status1" id="unit_status1">
              <div class="row">
                <div class="input-style-1">
                  <div class="column">
                    <label for="unit_name_update">Unit Name</label>
                    <input type="text" class="form-control" name="unit_name_update" id="unit_name_update">
                  </div>
                  <div class="column">
                    <label for="unit_plate">Unit Plate Number</label>
                  ...
                  <div class="column">
                    ...
                    <input type="number" class="form-control" name="unit_std_update" id="unit_std_update" readonly>
                  </div>
                </div>
              </div>
            </form>
          ...
          <div class="modal-footer">
            <button class="btn btn-primary btn-sm" name="submit" id="UpdateTruck">Update Unit</button>
            <button class="btn btn-success btn-sm" name="submit" id="FixUnit">Confirm</button>
            <button class="btn btn-warning btn-sm" name="submit" id="shopUnit">Rescue Unit</button>
            <button class="btn btn-danger btn-sm" name="submit" id="disposeUnit">Shop Unit</button>
    ...
    <script src="js/truck-dispatch.js"></script>
    <!-- solar icons -->
    <script src="https://cdn.jsdelivr.net/npm/iconify-icon@1.0.8/dist/iconify-icon.min.js"></script>
    <script>
      $(document).on("click", ".editBtn", function () {
        let unitStatus = $(this).data("status");

        $("#unit_status1").val(unitStatus);

        // Hide Confirm if status is Good or Dispatch
        if (unitStatus === "Good" || unitStatus === "Dispatch") {
            $("#FixUnit").hide();
        } else {
            $("#FixUnit").show();
        }
