...
$whereClauses[] = "d.d_trailer IS NOT NULL AND d.d_trailer <> ''";
...
if (!empty($_POST['trailer'])) {
    ...
    $whereClauses[] = "d.d_trailer = '$trailer'";
}
...
if ($result && ($result)->rowCount() > 0) {
    while ($row = ($result)->fetch()) {
        ...
        // Trailer ONLY (no unit)
        $sub_array[] = '
          <div class="d-flex align-items-center">
            <i class="bi bi-truck me-2"></i>
            <div>
              <h6 class="mb-0 fw-bolder">' . htmlspecialchars($row['d_trailer']) . '</h6>
            </div>
          </div>';
