...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($customer)) {
        $query .= " AND d.costumer = '" . pt_pg_escape($conn, $customer) . "'";
    }
    ...
    $sheet->setTitle('Truck Report');

    // Header row
    $headers = [
        'Truck', 'Driver', 'date', 'Start Location', 'Last Location',
    ...
    if ($result && ($result)->rowCount() > 0) {
        while ($row = ($result)->fetch()) {
            $sheet->setCellValue('A' . $rowNum, $row['d_truck']);
    ...
    // Export file
    // -------------------------------
    $fileName = 'Truck_Report_' . date('Ymd_His') . '.xlsx';
